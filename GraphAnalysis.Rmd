...
# plotting example of thresholded matrix
```{r, include=FALSE}
...
plot(g,layout = layout.circle, vertex.size=(strength(g)),edge.width=E(alpha)$weight)
```
# plotting example of minimal spanning tree
```{r, include=FALSE}
g=mst(alpha)
plot(g,vertex.size=(strength(g)))
```

# plotting example of thresholded matrix with colour attributes
```{r, include=FALSE}
col_fun = colorRamp2(c(min(E(g)$weight),max(E(g)$weight)), c("darkblue","darkred"))

weights <- sort(E(alpha)$weight, decreasing=TRUE)
threshold <- weights[length(weights)*0.2]
g=delete.edges(alpha, which(E(alpha)$weight <=threshold))
E(g)$color = col_fun(E(g)$weight)
plot(g,layout = layout.circle, vertex.size=(strength(g)),edge.width=E(g)$color)
```
